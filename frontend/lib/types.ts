...
// Where the board's islands and treaties came from. "loading" is the state
// before the first read resolves, and it is deliberately NOT the seed: see the
// note on the initial state in store.ts for why the board starts empty.
// "live" and "empty" both mean the deployed contract answered the read;
// "empty" means it answered and nothing has been founded on it yet.
// "simulated" means the contract could not be reached, so the reviewer-mode
// seed data is on screen instead.
export type StateSource = "loading" | "live" | "empty" | "simulated";
